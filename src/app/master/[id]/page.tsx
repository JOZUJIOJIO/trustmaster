  ...
  return (
    <div className="animate-pulse">
      <div className="text-center py-6">
        <div className="w-20 h-20 rounded-full bg-white/[0.05] mx-auto" />
        <div className="h-5 bg-white/[0.05] rounded w-40 mx-auto mt-3" />
        <div className="h-3 bg-white/[0.05] rounded w-32 mx-auto mt-2" />
      </div>
...
export default function MasterProfile({
    ...
    return (
      <div className="min-h-screen bg-[#0a0814]">
        <div className="lg:hidden flex items-center h-11 px-4 border-b border-amber-400/10 sticky top-0 bg-[#0a0814] z-10">
          ...
          <span className="font-semibold text-[15px] text-amber-100">{t("detail.profile")}</span>
        </div>
        <DetailSkeleton />
      </div>
    );
  ...
  if (!master) {
    return (
      <div className="flex items-center justify-center h-screen text-amber-200/40 bg-[#0a0814]">
        Master not found
      </div>
    );
  }
      ...
      <header className="hidden lg:block sticky top-0 z-50 bg-[#0a0814]/80 backdrop-blur-md border-b border-amber-400/10">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-amber-200/40">{t("detail.profile")}</span>
          </div>
          <LanguageSwitcher />
        </div>
      </header>
      ...
      <main className="lg:max-w-6xl lg:mx-auto lg:px-6 lg:py-10 pb-24 lg:pb-0">
        <div className="lg:grid lg:grid-cols-3 lg:gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="lg:bg-white/[0.03] lg:rounded-2xl lg:border lg:border-amber-400/10 lg:overflow-hidden lg:sticky lg:top-24">
              {/* Profile Header */}
              <div className="text-center py-6 bg-gradient-to-b from-amber-900/10 to-transparent lg:px-6">
                <div className="w-20 h-20 lg:w-24 lg:h-24 rounded-full bg-amber-900/20 border-2 border-amber-400/20 mx-auto flex items-center justify-center text-4xl lg:text-5xl overflow-hidden">
                  {master.avatar_url ? (
                    <Image
                ...
                <h1 className="text-lg lg:text-xl font-bold mt-3 text-amber-100">{name}</h1>
                {locale !== "th" && (
                  <p className="text-sm text-amber-200/40">{master.name_th}</p>
                )}
                {locale === "th" && (
                  <p className="text-sm text-amber-200/40">{master.name}</p>
                )}
                <p className="text-xs text-amber-200/70 mt-1">
                  {specialties.join(" · ")}
                </p>
                {master.verified && (
                  <span className="inline-block mt-2 px-3 py-1 bg-green-900/20 text-green-400/80 text-[11px] rounded-full border border-green-500/20">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-2 gap-3 px-4 py-4 lg:px-6">
                {[
                  ...
                  <div key={stat.label} className="bg-amber-900/10 rounded-xl p-3 text-center border border-amber-400/10">
                    <div className="text-lg font-bold text-amber-300">{stat.value}</div>
                    <div className="text-[11px] text-amber-200/40">{stat.label}</div>
                  </div>
                ))}
              </div>
              ...
              {hasLineId && (
                <div className="hidden lg:block px-6 pb-6">
                  <a
                    ...
                    className="block w-full bg-gradient-to-r from-amber-700 via-amber-600 to-amber-700 text-white text-center py-3.5 rounded-xl font-semibold text-[15px] transition-colors"
                  >
          ...
          <div className="lg:col-span-2 space-y-0 lg:space-y-6">
            {/* About */}
            <div className="px-4 py-4 border-b border-amber-400/10 lg:bg-white/[0.03] lg:rounded-2xl lg:border lg:border-amber-400/10 lg:p-6">
              <h2 className="text-sm lg:text-base font-semibold text-amber-200/80 mb-2">{t("detail.about")}</h2>
              <p className="text-[13px] lg:text-sm text-amber-200/60 leading-relaxed">
                {description}
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                <span className="text-[11px] lg:text-xs bg-white/[0.05] text-amber-200/60 px-2 py-1 rounded-full border border-white/5">
                  📍 {location}
                </span>
                <span className="text-[11px] lg:text-xs bg-white/[0.05] text-amber-200/60 px-2 py-1 rounded-full border border-white/5">
                  🕐 {master.availability}
                ...
                {master.languages.map((lang) => (
                  <span
                    key={lang}
                    className="text-[11px] lg:text-xs bg-white/[0.05] text-amber-200/60 px-2 py-1 rounded-full border border-white/5"
                  >
                    🌐 {lang}
                  </span>
                ))}
            ...
            {/* Price Info */}
            <div className="px-4 py-4 border-b border-amber-400/10 lg:bg-white/[0.03] lg:rounded-2xl lg:border lg:border-amber-400/10 lg:p-6">
              <h2 className="text-sm lg:text-base font-semibold text-amber-200/80 mb-2">{t("detail.pricing")}</h2>
              <div className="flex items-center justify-between bg-amber-900/10 rounded-xl p-4 border border-amber-400/10">
                <div>
                ...
                <div className="text-right">
                  <div className="text-base lg:text-lg font-bold text-amber-300">
            ...
            <div className="px-4 py-4 lg:bg-white/[0.03] lg:rounded-2xl lg:border lg:border-amber-400/10 lg:p-6">
              <h2 className="text-sm lg:text-base font-semibold text-amber-200/80 mb-3">
              ...
              <div className="lg:grid lg:grid-cols-2 lg:gap-4">
                {reviews.map((review) => (
                  <div
                    key={review.id}
                    className="mb-4 pb-4 border-b border-white/5 lg:mb-0 lg:pb-0 lg:border-b-0 lg:bg-white/[0.03] lg:rounded-xl lg:p-4"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[13px] font-semibold text-amber-200/70">
                        {review.user_name}
                      </span>
                    ...
                    <p className="text-[13px] text-amber-200/60 mt-1.5 leading-relaxed">
