...
请严格按以下 JSON 格式返回，每个字段 250-400 字（actionItems 可简短，每条30-50字）：
{
  ...
  "actionItems": "本月行动清单 — 基于当前流年大运和命盘格局，给出5条具体可执行的行动建议。每条格式为「领域：具体行动」。例如：「事业：本月适合主动争取项目负责人角色，尤其周三周四精力最旺」「健康：每周至少3次有氧运动，晚上11点前入睡养肝」「财务：本月偏财运旺，可小额试水投资，但单笔不超过月收入10%」。要具体到可以立即执行的程度，不要空话。"
}
