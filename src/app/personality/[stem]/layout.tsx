...
const STEM_META: Record<string, { title: string; desc: string }> = {
  甲: { title: "甲木 (Jia Wood) Personality", desc: "甲木 Day Master: Like a towering tree — leadership, ambition, and growth. Discover your work style, relationships, and personal rhythm." },
...
export async function generateMetadata({ params }: { params: Promise<{ stem: string }> }): Promise<Metadata> {
  ...
  if (!meta) {
    return { title: "Five Elements Personality Analysis" };
  }

  return {
    title: meta.title,
    description: meta.desc,
    keywords: [`${decoded} Day Master`, "Five Elements personality", `${decoded}木 性格`, "Four Pillars", "个人图谱分析"],
