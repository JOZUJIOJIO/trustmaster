...
export default async function AdminOrdersPage() {
  ...
  return (
    <AdminShell title="订单管理" description="查看 Stripe 与 Telegram Stars 支付订单、套餐、金额、用户与支付状态。">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1080px] w-full text-left text-sm">
            <thead className="text-xs text-stone-500">
              <tr><th className="py-2">客户</th><th>用户</th><th>渠道</th><th>套餐</th><th>金额</th><th>状态</th><th>支付单号</th><th>创建</th><th>支付</th></tr>
            </thead>
            <tbody className="divide-y divide-stone-100">
              {result.rows.map((order) => (
                <tr key={order.id}>
                  <td className="py-3 pr-3">{order.customer_email || order.user_name || "-"}</td>
                  <td className="pr-3 font-mono text-xs">{order.user_id || (order.telegram_user_id ? `tg:${order.telegram_user_id}` : "-")}</td>
                  <td className="pr-3">{order.payment_provider === "telegram_stars" ? "Telegram Stars" : "Stripe"}</td>
                  <td className="pr-3">{order.tier}</td>
                  <td className="pr-3">{order.currency === "XTR" ? formatMoney(order.amount || 0, "XTR") : formatMoney((order.amount || 0) / 100, order.currency || "USD")}</td>
              ...
              {result.rows.length === 0 && <tr><td className="py-8 text-center text-stone-500" colSpan={9}>暂无订单数据</td></tr>}
            </tbody>
          </table>
        </div>
      </AdminPanel>
